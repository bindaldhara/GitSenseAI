...
from rag.reranker import rerank_retrieved_chunks
...
def _chunks_from_documents(documents: list[Document]) -> list[RetrievedChunk]:
    return [_document_with_rank_score(doc, rank) for rank, doc in enumerate(documents, start=1)]
...
def retrieve_repository_context(
    ...
    *,
...
) -> tuple[list[RetrievedChunk], RetrievalMode, bool]:
    """Retrieve chunks using hybrid (BM25 + vector) or vector-only search, optionally reranked."""
    ...
    retrieve_k = candidate_k if rerank_enabled else top_k

    vector_retriever = RepositoryVectorRetriever(repository_id=repository_id, k=retrieve_k)

    if not hybrid_enabled:
        documents = vector_retriever.invoke(question)[:retrieve_k]
        chunks, mode = _chunks_from_documents(documents), "vector"
    elif not bm25_index_exists(repository_id):
        logger.info(
        ...
        bm25_retriever = _build_bm25_retriever(repository_id, k=retrieve_k)
        if bm25_retriever is None:
            ...
            fused_documents = ensemble.invoke(question)[:retrieve_k]
            chunks = _chunks_from_documents(fused_documents)
            mode = "hybrid"
            logger.info(
    ...
    if rerank_enabled and chunks:
        chunks = rerank_retrieved_chunks(question, chunks, top_k=top_k)
    elif not rerank_enabled:
        chunks = chunks[:top_k]

    return chunks, mode, rerank_enabled and bool(chunks)
